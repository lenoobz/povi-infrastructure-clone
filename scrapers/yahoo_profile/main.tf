locals {
  ...
  step_functions = {
    "step_1" = {
      function_name            = "yahoo-asset-profile-scraper"
    ...
    "step_2_a" = {
      function_name            = "yahoo-profile-norm-countries"
      function_runtime         = "go1.x"
      function_handler         = "main"
      function_timeout_seconds = 900
      api_log_retention_days   = 3
    ...
    "step_2_b" = {
      function_name            = "yahoo-profile-norm-sectors"
      function_runtime         = "go1.x"
      function_handler         = "main"
      function_timeout_seconds = 900
      api_log_retention_days   = 3
...
resource "aws_sfn_state_machine" "state_machine" {
  ...
  definition = <<EOF
  {
    ...
    "StartAt": "YahooAssetProfileScraper",
    "States": {
      "YahooAssetProfileScraper": {
        "Type": "Task",
        "Resource": "${module.scraper_step_functions["step_1"].function_arn}",
        "ResultPath": "$.tickers",
        "Next": "YahooProfileProcessData"
      },
      "YahooProfileProcessData": {
        "Type": "Parallel",
        "End": true,
        "Branches": [
          {
...
module "scrape_yahoo_price_cron" {
